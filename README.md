...
一共六章，也就是六级月光阶梯，每级会有不同的攀登条件，每踏上一级阶梯之后，会获取剧情碎片或观看CV（视剧情工作量而定）

每个章节的功能牌对应这个章节的月相，然后通过前五个章节，要求玩家进行牌组构建，考验他们对各个月相对应的功能牌与过牌、环境和策略的平衡，然后到第六章也就是阶梯六的时候，需要玩家使用自己构建好的牌组，与一个和玩家牌组完全一样的ai对战，也就是要求玩家战胜自己，与自己和解
...
* **牌组构建**：主要为构建功能牌
    ...
    * 后续只需将基础牌和功能牌各补至3张
    * 基础牌手牌上限为3，功能牌手牌上限会随着游戏进程而改变，初始也为3
